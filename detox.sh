...
trap 'catch $LINENO' ERR
    ...
    printf "%s${NC}\n\n" "==> https://gitlab.com/jayvialv/universally_detox"
}
...
main() {
    ...
    printf "\n${BORANGE}%s\n"                       "================================================"
    printf "%s\n"                                   "#                                              #"
    printf "%-8s${NC}%s${BORANGE}%8s\n"             "#"     "OnePlus Nord N100 T-Mobile Detoxifier & Universal Android Debloater"                      "#"
    printf "%s\n"                                   "#                                              #"
    printf "%-${PAD}s${BRED}%s${BORANGE}%${PAD}s\n" "#"               "${VERSION}"                 "#"
    printf "%s\n"                                   "#                                              #"
    printf "%s${NC}\n\n"                            "================================================"

    printf "${Bold}%s\n\n"                          "Make sure you entered 'adb shell box --install -s /data/local/tmp' to install BusyBox!!"
...
restore_apks() {
    ...
    adb shell "/data/local/tmp/su -c \"mount -o rw,remount $SYSTEM_MOUNT_POINT\""
    ...
    adb shell "/data/local/tmp/su -c \"mount -f -o ro,remount $SYSTEM_MOUNT_POINT\""
